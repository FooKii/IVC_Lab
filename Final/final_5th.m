...
rec_2nd_frame_NCS_cell = transform_ycbcr2cell_5(rec_2nd_frame_ycbcr);
frame_NCS_cell = transform_ycbcr2cell_5(frame_ycbcr{2});
...
for i = 2: length(frame_ycbcr)
    ...
    im_NCS_cell = transform_ycbcr2cell_5(im);
    rec_im_NCS_cell = transform_ycbcr2cell_5(rec_im);
    for inloop = 1:3
...
%     recon_frame_rgb = NewColorSpace_dec(recon_frame_NCS_up, T_pca_im, Offset_pca_im, means_im);
    recon_frame_NCS = recon_frame_NCS_up;
    recon_frame_NCS(:,:,4) = im_NCS_cell{4};
    recon_frame_NCS(:,:,5) = im_NCS_cell{5};
    recon_frame_NCS(:,:,6) = im_NCS_cell{6};
    recon_frame_rgb = NCS_block_dec(recon_frame_NCS);
