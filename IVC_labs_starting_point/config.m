...
ParameterStruct(1).name='lena.tif - starting algorithm';                % Name of the investigated compression mode
ParameterStruct(1).input_image_filename='data/images/monarch.tif';         % Input image file name
...
ParameterStruct(1).output_stream_filename='data/compressed/lena.ivc';   % Output file name (Compressed)
...
ParameterStruct(1).reconstructed_image_filename='data/reconstructed/lena.tif';    % Output file name (Reconstructed)

%-------------------------------
% E2
%-------------------------------
pmfimg = double(imread('data/images/lena.tif'));
%pmfimg = pmfimg/256;
pmf = stats_marg(pmfimg);
H = calc_entropy(pmf,pmf);
% Gather info from 3 images
gimg1 = double(imread('data/images/lena.tif'));
%pmf1 = stats_marg(gimg1);
gimg2 = double(imread('data/images/sail.tif'));
%pmf2 = stats_marg(gimg2);
gimg3 = double(imread('data/images/smandril.tif'));
%pmf3 = stats_marg(gimg3);
size(gimg1(:))
big_img = [gimg1(:);gimg2(:);gimg3(:)];
pmf_gather = stats_marg(big_img);
H_common = calc_entropy(pmf, pmf_gather);
%--------------------------
